...
create_user_domain() {
  ...
  read response
  finish=-1
  count=0 # avoid an infinite loop of asking for response from EOF
  while [ "$finish" = '-1' -a $count -lt 100 ]
  do
    finish="1"
    if [ "response" = '' ];
    then
  ...
  then
     ...
     $JAVA_HOME/bin/java -Djava.security.egd=file:///dev/urandom -Djava.endorsed.dirs="${MW_HOME}/oracle_common/modules/endorsed" \
...
cpset=${JAVA_HOME}/lib/tools.jar${CLASSPATHSEP}${ANT_HOME}/lib/ant-launcher.jar${CLASSPATHSEP}${ANT_HOME}/lib/ant.jar

"${JAVA_HOME}/bin/java" -cp ${cpset} -Dant.home=${ANT_HOME} \
